export function getWeeklyRetrospectiveEmail(data: {
  ...
  const weekScore = Math.min(100, Math.round(
    (taskCompletionRate * 0.5) + (habitAvgPct * 0.5) + (isAllComplete ? 10 : 0)
  ));
...
<head>
  ...
  <title>Your Week in Review — SlowDesk</title>
</head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background-color:${colors.white};border-radius:20px;overflow:hidden;border:1px solid ${colors.border};">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#2c2416 0%,#4a3728 60%,#c1623f 100%);padding:40px 36px 32px;text-align:center;">
            ...
            <h1 style="margin:0 0 8px;font-size:32px;font-weight:800;color:#ffffff;line-height:1.15;letter-spacing:-0.5px;">
              Hey ${escapeHtml(displayName)},
            </h1>
            <p style="margin:0 0 28px;font-size:17px;color:rgba(255,255,255,0.75);line-height:1.5;">
              ${tierMessage}
            </p>
            <!-- Score badge -->
            ...
            <div style="margin-top:16px;display:inline-block;background:${tierColor}22;border:1px solid ${tierColor}55;border-radius:20px;padding:6px 16px;">
              <span style="font-size:13px;font-weight:700;color:${tierColor};">${tierEmoji} ${tier}</span>
            </div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:32px 36px 0;">
            ...
            ${isAllComplete ? `
            <!-- ── PERFECT WEEK BANNER ── -->
            <div style="background:linear-gradient(135deg,rgba(122,158,126,0.12) 0%,rgba(122,158,126,0.06) 100%);border:1.5px solid ${colors.success}44;border-radius:14px;padding:22px 24px;margin-bottom:28px;text-align:center;">
              <div style="font-size:36px;margin-bottom:8px;">🏆</div>
              <div style="font-size:16px;font-weight:700;color:${colors.success};margin-bottom:6px;">Perfect Week</div>
              <p style="margin:0;font-size:14px;color:${colors.textMuted};line-height:1.6;">Every task done. Every habit kept. You didn't just have a good week — you had the best kind.</p>
            </div>
            ` : ''}
            ...
            <div style="margin-bottom:28px;">
              <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:14px;">
                <tr>
                  ...
                  <td align="right"><span style="font-size:12px;color:${colors.textFaint};">${completedTasksCount} task${completedTasksCount !== 1 ? 's' : ''}</span></td>
                </tr>
              </table>
              <div style="background:${colors.bgCard};border-radius:12px;padding:4px 0;border:1px solid ${colors.border};">
                ...
                <div style="padding:11px 16px;${i < Math.min(completedTasks.length, 6) - 1 ? `border-bottom:1px solid ${colors.border};` : ''}display:flex;align-items:center;gap:10px;">
                  <span style="color:${colors.success};font-size:13px;flex-shrink:0;">✓</span>
                  <span style="font-size:14px;color:${colors.text};font-weight:500;flex:1;">${escapeHtml(task.title)}</span>
                  ${task.project ? `<span style="font-size:11px;color:${colors.textFaint};background:${colors.bg};border:1px solid ${colors.border};border-radius:6px;padding:2px 8px;flex-shrink:0;">${escapeHtml(task.project)}</span>` : ''}
                </div>
                `).join('')}
                ${completedTasks.length > 6 ? `
                <div style="padding:10px 16px;text-align:center;font-size:12px;color:${colors.textFaint};">+ ${completedTasks.length - 6} more</div>
                ` : ''}
              </div>
            </div>
            ` : `
            <!-- ── ZERO TASKS ── -->
            <div style="background:${colors.bgCard};border-radius:14px;padding:24px;margin-bottom:28px;text-align:center;border:1px solid ${colors.border};">
              <div style="font-size:32px;margin-bottom:10px;">🌿</div>
              <p style="margin:0;font-size:14px;color:${colors.textMuted};line-height:1.7;">Quiet week — and that's okay. The best weeks start with one small task. <strong style="color:${colors.text};">What's the one thing you'll finish tomorrow?</strong></p>
            </div>
            `}
            ...
            <div style="background:linear-gradient(135deg,rgba(201,148,58,0.08) 0%,rgba(201,148,58,0.04) 100%);border:1.5px solid ${colors.warning}55;border-radius:14px;padding:20px 22px;margin-bottom:28px;">
              ...
              <div style="padding:7px 0;font-size:14px;color:${colors.text};border-bottom:1px solid ${colors.warning}22;">
                <span style="color:${colors.warning};margin-right:8px;">→</span>${escapeHtml(task.title)}
              </div>
              `).join('')}
            </div>
            ` : ''}
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0">
                ...
                <tr>
                  <td style="padding:6px 0;vertical-align:top;">
                    <span style="font-size:13px;color:${colors.primary};font-weight:700;margin-right:8px;">01</span>
                    <span style="font-size:14px;color:${colors.text};">Make <strong>${escapeHtml(strugglingHabits[0].name)}</strong> effortless — set a reminder or tie it to another habit.</span>
                  </td>
                </tr>
                ` : ''}
                ${avoidedTasks.length > 0 ? `
                <tr>
                  <td style="padding:6px 0;vertical-align:top;">
                    <span style="font-size:13px;color:${colors.primary};font-weight:700;margin-right:8px;">${strugglingHabits.length > 0 ? '02' : '01'}</span>
                    <span style="font-size:14px;color:${colors.text};">Clear one overdue task — even 20 minutes makes a difference.</span>
                  </td>
                </tr>
                ` : ''}
                <tr>
                  <td style="padding:6px 0;vertical-align:top;">
                    <span style="font-size:13px;color:${colors.primary};font-weight:700;margin-right:8px;">${(strugglingHabits.length > 0 ? 1 : 0) + (avoidedTasks.length > 0 ? 1 : 0) + 1 < 10 ? '0' : ''}${(strugglingHabits.length > 0 ? 1 : 0) + (avoidedTasks.length > 0 ? 1 : 0) + 1}</span>
                    <span style="font-size:14px;color:${colors.text};">${completedTasksCount >= 5 ? 'Great momentum — protect your energy, not just your schedule.' : 'Start Monday with one clear intention. One is enough.'}</span>
                  </td>
                </tr>
              </table>
            </div>
            ...
            <div style="text-align:center;padding-bottom:36px;">
              <a href="https://slowdesk.app" style="display:inline-block;background:${colors.primary};color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:10px;font-size:15px;font-weight:700;letter-spacing:0.01em;">${ctaText}</a>
...
function getPerformanceTier(score: number, completedCount: number): {
  tier: string; tierColor: string; tierEmoji: string; tierMessage: string;
} {
  if (score >= 90) return {
    tier: 'Legendary',
    tierColor: '#f0c040',
    tierEmoji: '🏆',
    tierMessage: "You didn't just have a productive week — you had a great one.",
  };
  if (score >= 75) return {
    tier: 'On Fire',
    tierColor: '#e07040',
    tierEmoji: '🔥',
    tierMessage: "Strong work. You're building something real.",
  };
  if (score >= 55) return {
    tier: 'Solid',
    tierColor: '#7a9e7e',
    tierEmoji: '💪',
    tierMessage: "Good progress. Consistency beats intensity every time.",
  };
  if (score >= 35) return {
    tier: 'Getting There',
    tierColor: '#c9943a',
    tierEmoji: '📈',
    tierMessage: completedCount > 0
      ? "Not your best week — but you showed up. That counts."
      : "Every week is a chance to reset. Next one starts tomorrow.",
  };
  return {
    tier: 'Rest Week',
    tierColor: '#a89f93',
    tierEmoji: '🌿',
    tierMessage: "Slow weeks happen. The important thing is you're here.",
  };
}

function getMotivationalQuote(score: number): { text: string; author: string } {
  ...
  const pool = score >= 70 ? highScoreQuotes : score >= 40 ? midScoreQuotes : lowScoreQuotes;
