...
export default function AuthModal({ open, onClose, initialMode = 'login' }: Props) {
  ...
  useEffect(() => { if (open) { setError(''); setName(''); setEmail(''); setPassword(''); } }, [open]);
      ...
      <div style={{
        ...
        <div style={{ padding: '32px 32px 28px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {error && (
              <div style={{ padding: '10px 14px', borderRadius: 8, fontSize: 13, background: 'rgba(224,92,60,0.08)', color: '#e05c3c', border: '1px solid rgba(224,92,60,0.2)' }}>{error}</div>
            )}
          ...
          <div style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: 'var(--ink-faint)' }}>
            ...
            <button onClick={() => { setMode(m => m === 'login' ? 'signup' : 'login'); setError(''); }} style={{
              background: 'none', border: 'none', cursor: 'pointer',
              color: 'var(--accent)', fontWeight: 600, fontSize: 13, fontFamily: 'var(--font-sans)',
            }}>
